import logging
from scrapper.modules import TwitterScraper
...
logging.basicConfig(level=logging.DEBUG, filename='/root/NewsLix/debug.log')
...
def tweet_worker_new(project_name, keyword, process_tweete_obj):
    try:
        logging.info('Inside tweet_worker_new ----- ')
        print(dir(TwitterScraper))
        #selenium_obj = SeleniumPageSourceScraper()
        bearer_token = 'AAAAAAAAAAAAAAAAAAAAAKClsgEAAAAAi2wOpv6VZ%2Fp4u86NbKOYsKSSBIg%3DHfslyThTDyY1pZCx37EPn3u3OBSpOkgCo4YxSewc7pM7ngQkLj'
        print(type(TwitterScraper)) 
        scraper = TwitterScraper(bearer_token)

        # Example to search for tweets containing 'electric vehicle' from the last hour, max 10 results
        logging.info(len(keyword))
        all_tweets = scraper.get_tweets(keyword=keyword)
        logging.info(len(all_tweets))
        #logging.info(all_tweets)
        save_file(data=all_tweets, is_tweet=True, project_name=project_name)
        for each_tweet in all_tweets:
            logging.info(each_tweet)
            try:
                tweet = process_tweete_obj.get_tweets(tweet_obj=each_tweet)
...
def parallel_tweet_scrape():
    ...
    for each_project in projects:
        ...
        process_tweete_obj = ProcessTweet(target_keywords=target_keywords, target_states=target_states)

        for each_keyword in target_keywords:
            for each_state in target_states:
                #results.append(pool.apply_async(tweet_worker_new, args=(project_name, each_keyword, process_tweete_obj)))
                tweet_worker_new(project_name, each_keyword, process_tweete_obj)
