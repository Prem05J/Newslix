...
class ProcessTweet:
    ...
    def get_tweets(self, tweet_obj):
        ...
        if date_publish:
            ...
            naive_datetime = datetime.datetime.strptime(date_publish, iso_format)

            # Make the datetime object timezone-aware in UTC
            aware_datetime = timezone.make_aware(naive_datetime, timezone.utc)

            # If you need to convert this to another timezone, do so here
            date_publish = aware_datetime
        else:
            date_publish = timezone.now()
